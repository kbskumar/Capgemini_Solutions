...
export default function Store(props) {
	...
	console.log(props);
	const data = props.location.state.sear;
	let temp = [];
	...
	const unSaveListener = async (e) => {
		...
		await fetch("http://localhost:9006/api/deletesavedcoupon/",{
			...
			let coupons = JSON.parse(sessionStorage.getItem("savedcoupons"));
			coupons.splice(coupons.findIndex((va)=>va===e.id),1);
			sessionStorage.setItem("savedcoupons",JSON.stringify(coupons));
			setRe(true);
			return;
		})
						...
						return (
							...
							>
								...
								<Card.Body>
									...
									{sessionStorage.getItem("loggedin")&&sessionStorage.getItem("loggedin")!=='false'?(JSON.parse(sessionStorage.getItem("savedcoupons")).includes(val.id) ?
